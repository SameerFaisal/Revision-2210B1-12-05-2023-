// let items=["apple","mango","orange","peach"]
// alert(`Available items are ${items}`)
// let user=prompt("What do you want to buy?")
// for(let i=0;i<items.length;i++){
//     if(user===items[i]){
//         alert(`${user} is available at index ${i} in our bakery`)
//         bill(user)
//         break
//     }
//     else{
//         alert(`We are sorry. ${user} is not available in our bakery`)
//         break
//     }
// }

// function bill(user){
//     price=0
//     if(user.toLowerCase()==="apple"){
//         price=100
//     }
//     else if(user.toLowerCase()==="mango"){
//         price=200
//     }
//     else if(user.toLowerCase()==="orange"){
//         price=150
//     }
//     else if(user.toLowerCase()==="peach"){
//         price=250
//     }
//     else{
//         alert("Item not available")
//     }
//     let quantity=prompt("Enter quantity of item")
//     let total=price*quantity
//     alert(`Total cost of your order is ${total}`)

// }

//DOM
// function fullName(){
//     let fn=document.getElementById("fn").value
//     let ln=document.querySelector("#ln").value
//     let fullName=fn+" "+ln
//     document.querySelector("#output").innerHTML=fullName
   

// }

// function add(){
//     let num1=document.getElementById("num1").value
//     let num2=document.getElementById("num2").value
//     let sum=parseInt(num1)+parseInt(num2)
//     document.getElementById("add").innerHTML=sum

// }
// function sub(){
//     let num1=document.getElementById("num1").value
//     let num2=document.getElementById("num2").value
//     let sub=parseInt(num1)-parseInt(num2)
//     document.getElementById("sub").innerHTML=sub

// }
// function mul(){
//     let num1=document.getElementById("num1").value
//     let num2=document.getElementById("num2").value
//     let mul=parseInt(num1)*parseInt(num2)
//     document.getElementById("mul").innerHTML=mul

// }
// function div(){
//     let num1=document.getElementById("num1").value
//     let num2=document.getElementById("num2").value
//     let div=parseInt(num1)/parseInt(num2)
//     document.getElementById("div").innerHTML=div

// }

// function changeImage(){
// document.getElementById("tree").src="https://media.istockphoto.com/id/184944186/photo/quaid-e-azam.jpg?s=612x612&w=0&k=20&c=7mRHDKfBWbpmiTto_w_oMm4EeboU9tEDO_JXke01P5I="
// }
// function reverse(){
    document.getElementById("tree").src="data:image/jpeg;base64,"
// }
// function apply(){
//     // document.getElementById("hw").setAttribute("class","red")
// document.getElementById("hw").classList.add("red")
// }
// function remove(){
//     document.getElementById("hw").classList.remove("red")
// }
// function apply(){
//    console.log( document.getElementById("hw").className)
// }

function sqrt(){
    var num = document.getElementById("num").value
    var result = num**(1/2)
    document.getElementById("output").innerHTML = result
}